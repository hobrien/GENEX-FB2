...
all_sample_info <- sample_info %>% mutate(PCW=floor(PCW)) %>% 
  ...
  select(Sample, FID, IID, VCF, everything()) %>%
...
Omni25_regions_With_Code <- read_delim("../Genotyping/1KGP_Heath/Omni25.regions_With_Code.txt", 
  ...
  select(IID=X1, POP=X2.x, SuperPop=X2.y, PopNum=X3.x)
...
``` {r PCA}
PCA = read.table("../Genotyping/Merged/FB_all_1kg_pca.eigenvec", h=F, as.is=T) 

PCA <- full_join(all_sample_info, PCA, by=c('IID' = 'V2'))

ggplot(filter(PCA, is.na(VCF)), aes(x=V3, y=V4, colour=SuperPop)) +
  geom_point(alpha=.2) +
  geom_point(colour='black', size=.5, data=filter(PCA, !is.na(VCF))) +
  geom_text(aes(label=Sample, x=V3+.001), colour='black', hjust=0, data=filter(PCA, Sample=='A225')) +
  xlab('PCA1') +
  ylab('PCA2') +
  main_theme() +
  theme(legend.position=c(.9,.8)) +
...
filter(PCA, SuperPop=='EUR') %>% ggplot(aes(x=V3, y=V4, colour=POP)) +
  geom_point(alpha=.2) +
  geom_point(colour='black', size=.5, data=filter(PCA, !is.na(VCF))) +
  geom_text(aes(label=Sample, x=V3+.001), colour='black', hjust=0, data=filter(PCA, Sample=='A225')) +
  xlab('PCA1') +
  ...
  scale_y_continuous(limits=c(0.015,.03))
```
